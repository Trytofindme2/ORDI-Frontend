...
const FriendList = ({ friends,makeUnFriend }) => {
  const { theme } = useContext(ThemeContext);
  const isDark = theme === 'dark';
  const [showAll, setShowAll] = useState(false);
  const navigate = useNavigate();
  const Friends = showAll ? friends : friends.slice(0, 5);
        ...
        <ul className={`divide-y transition-colors ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
          {Friends.map((friend) => (
            <li
              ...
              <button onClick={()=>makeUnFriend(friend.id)} className="rounded-3xl bg-blue-500 px-4 py-1.5 sm:px-5 sm:py-2 text-white text-sm sm:text-base transition-all hover:bg-blue-600 focus:bg-blue-400">
                UnFriend
              </button>
            </li>
          ))}
