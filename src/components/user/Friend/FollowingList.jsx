...
const FollowingList = ({ followings, makeUnFollow }) => {
  ...
  const Followings = showAll ? followings : followings.slice(0, 5);
    ...
    <div className={`w-full pb-6 transition-colors duration-300 ${isDark ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      ...
      <div className="pt-2 px-4 sm:px-6">
        <h2 className="text-lg sm:text-xl font-semibold mb-2">
          My followings ({followings.length})
        </h2>
        ...
        <ul className={`divide-y transition-colors ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
          {Followings.map((following) => (
            <li
              ...
              <img onClick={() => SearchUserProfile(following.email, following.id)}
                className="w-12 h-12 rounded-full object-cover"
                src={`http://localhost:8080/uploads/${following.profile_URl}`}
              ...
              <button onClick={() => makeUnFollow(following.id)} className="rounded-3xl bg-blue-500 px-4 py-1.5 sm:px-5 sm:py-2 text-white text-sm sm:text-base transition-all hover:bg-blue-600 focus:bg-blue-400">
                UnFollow
              </button>
            </li>
          ))}
